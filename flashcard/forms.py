...
class FlashCardForm(forms.ModelForm):
    question_image = forms.ImageField(required=False, widget=forms.widgets.FileInput(attrs={'accept':'image/*;capture=camera'}) )
    answer_image = forms.ImageField(required=False, widget=forms.widgets.FileInput(attrs={'accept':'image/*;capture=camera'}) )
    class Meta:
        model = FlashCard 
        exclude = ("object_owner", 'section', 'vote_average')